		...
		if(name.length){
			...
			if(!$('.' + name + '_frame[data-trigger="' + encTriggerAttrs + '"]').length){
				if(_this.attr('disabled') != 'disabled'){
					...
					$('body').append('<div class="' + name + '_frame jqmWindow" style="max-width: 100vw; width: 500px;" data-trigger="' + encTriggerAttrs + '"></div>');
...
$(document).ready(function(){
	...
	if(isMobile){
		...
		$(document).on('click', '*[data-event="jqm"]', function(e){
			...
			//if(window.matchMedia('(min-width:992px)').matches) - включаем всплывашку в мобильных браузерах
			if(window.matchMedia('(min-width:9px)').matches)
			{
				e.stopPropagation();
				$(this).jqmEx();
				$(this).trigger('click');
...
$(document).ready(function() {
		$('.publication__top-title').on('click', function (e) {
			e.preventDefault();
			$(".publication__top-list--main").slideToggle();
			$(this).toggleClass("active")
		})
})
...
console.log(123);
	if(window.innerWidth < 868) {
		$(".publication__top-list--main").slideDown(400);
	} else {
		$('.publication__top-list.publication__top-list--main').css({'display': 'block'})
	}
}
